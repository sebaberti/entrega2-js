 ...
 const saveLocalStorage = () => {
  ...
  const renderPizzas = e => {
      ...
      number.value= " ";
    }else {  card.innerHTML = `<div class="carda"><img src="${pizzaSearch(pizzas).imgSrc}" class="asd"/><h1> Pizza ${pizzaSearch(pizzas).nombre}</h1> <p>Ingredientes: ${pizzaSearch(pizzas).ingredientes}</p> <h3>$ ${pizzaSearch(pizzas).precio}</h3> <button type="submit" class="btnn">Comprar</button></div>`;
      number.value = " ";
